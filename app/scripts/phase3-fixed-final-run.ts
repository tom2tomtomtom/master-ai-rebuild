...
const logFile = resolve(logsDir, `phase3-fixed-${new Date().toISOString().replace(/:/g, '-')}.log`);
...
// ============================================================================
// FIXED PARSER LOGIC (from test-fixed-parser-lesson-02.ts)
// ============================================================================
...
async function insertStages(lessonId: string, stages: ParsedStage[]): Promise<boolean> {
  try {
    ...
    if (insertError) {
      log(`    ❌ Failed to insert stages: ${insertError.message}`);
      return false;
    }
...
async function processLesson(filename: string): Promise<LessonResult> {
  ...
  // Insert to database if PASS
  if (status === 'PASS') {
    const lessonId = await getLessonId(filename); // Pass filename instead of slug
    ...
    result.issues.push(`Quality ${avgScore}/100 - needs review`);
  } else {
    result.issues.push(`Quality ${avgScore}/100 - too low`);
  }
...
async function processAllLessons() {
  log('='.repeat(80));
  log('PHASE 3: FIXED PARSER - FINAL RUN');
  log('='.repeat(80));
  log(`Started: ${new Date().toISOString()}\n`);

  const lessonsPath = resolve(__dirname, '../../lessons');
  const files = readdirSync(lessonsPath)
  ...
  log(`✅ PASS: ${passResults.length} (inserted to database)`);
  log(`⚠️  WARN: ${warnResults.length} (need review)`);
  log(`❌ FAIL: ${failResults.length} (not inserted)\n`);

  const totalStages = results.reduce((sum, r) => sum + r.stages, 0);
  const avgQuality = Math.round(results.reduce((sum, r) => sum + r.quality, 0) / results.length);

  log(`Total stages created: ${totalStages}`);
  log(`Average quality: ${avgQuality}/100\n`);

  if (warnResults.length > 0) {
    log(`${'='.repeat(80)}`);
    log('WARN LESSONS (need review)');
    log(`${'='.repeat(80)}\n`);
  ...
  // Save results
  const resultsPath = resolve(logsDir, 'phase3-fixed-final-results.json');
  ...
  if (passResults.length >= 35 && avgQuality >= 90) {
    log(`✅ SUCCESS: ${passResults.length} lessons inserted with ${avgQuality}/100 avg quality!`);
  } else if (passResults.length > 0) {
    log(`⚠️  PARTIAL SUCCESS: ${passResults.length} lessons inserted, ${warnResults.length + failResults.length} need review`);
  } else {
    log(`❌ FAILED: No lessons inserted - all need fixes`);
  }
}

processAllLessons().catch(error => {
  log(`\n❌ FATAL ERROR: ${error}`);
  console.error(error);
  process.exit(1);
});
